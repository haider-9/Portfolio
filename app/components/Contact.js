...
const ContactOption = ({ icon, label, value, href, delay, color }) => {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.95 }}
    >
      <Card className={`bg-zinc-800/70 backdrop-blur-sm border-zinc-700/50 hover:border-${color}-500/50 hover:bg-zinc-800/90 transition-all duration-300 shadow-lg hover:shadow-xl hover:shadow-${color}-500/20`}>
  ...
  return (
    <section id="contact" className="max-w-6xl mx-auto px-4 py-24 relative">
      ...
      <div className="grid md:grid-cols-5 gap-10">
        ...
        >
          <Card className="bg-zinc-800/70 border-zinc-700/50 shadow-xl backdrop-blur-sm">
            <CardContent className="p-8">
              <h3 className="text-2xl font-semibold text-zinc-100 mb-6">Send a Message</h3>
              {feedback === "success" && (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <Input
                  ...
                  className="bg-zinc-700/60 border-zinc-600/50 text-white placeholder:text-zinc-400 focus:border-blue-500/50 focus:ring-blue-500/20 transition-colors"
                  required
                />
                <Input
                  type="email"
                ...
                <Textarea
                  ...
                  className="bg-zinc-700/60 border-zinc-600/50 text-white placeholder:text-zinc-400 focus:border-blue-500/50 focus:ring-blue-500/20 min-h-[120px] transition-colors resize-none"
                  required
                />
                <Button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full bg-blue-600/90 hover:bg-blue-700/90 disabled:opacity-70 text-white border-0 shadow-lg hover:shadow-blue-500/25 transition-all duration-300"
                >
